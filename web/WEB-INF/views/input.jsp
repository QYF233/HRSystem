...
<script type="text/javascript">
    $(function () {
        $.ajax({
            ...
            beforeSend: function () {
                $("form select").empty();
                $("form select").attr({"title": "正在加载数据..."});
            },
            success: function (result) {
                if (result.code == 100) {
                    $("form select").empty();
                    $("form select").attr({"title": "请选择部门"});
                    $.each(result.extend.pageInfo.list, function (index, dept) {
                        var optionEle = $("<option></option>").attr({"value": dept.id}).append(dept.deptName);
                        optionEle.appendTo($("form select"));
                    });
                    $("form select").selectpicker("refresh");
                    $("form select").parent().find("button").attr({"title": "请选择部门"});
                    $("form select").parent().find("div.filter-option-inner-inner").text("请选择部门");
                }
            }
        });
    });
